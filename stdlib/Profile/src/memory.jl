...
struct AllocationInfo
    ...
    alloc_time::Float64
    free_time::Float64
    alloc_stacktrace::StackTrace
    free_stacktrace::StackTrace
    allocsz::UInt64
    tag::UInt16
...
`@profile <expression>` runs your expression while keeping track of memory operations
(allocations, reallocations, deallocations). These are appended to an internal buffer of
backtraces.
"""
macro profile(ex)
    return quote
...
function read_and_coalesce_memprofile_data()
    ...
    if overflow & 0x01 != 0
        @warn """The memory profile backtrace buffer overflowed; profiling terminated
                 ...
                 `Profile.Memory.init()` with a larger buffer and/or larger delay."""
    end
    if overflow & 0x02 != 0
